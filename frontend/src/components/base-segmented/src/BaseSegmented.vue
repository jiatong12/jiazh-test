...
import { ElSegmented, ElSkeleton, ElSkeletonItem } from 'element-plus'
import { h } from 'vue'
...
const { componentRef, componentExposed } = useComponentExposed(ElSegmented, {
  // 自定义暴露属性
})
...
<template>
  <ElSkeleton animated :loading="dictApi.delayLoading" :rows="1" style="height:var(--el-component-size)">
    <template #template>
      <ElSkeletonItem variant="rect" class="w-full" />
    </template>
    <template #default>
      <component
        :is="h(ElSegmented, {}, $slots)"
        :ref="componentRef"
        :options="dictApi.result"
