...
const currentSiteID = ref(userStore.sites?.siteID)
...
const dimensionState = reactive({
  dimensionID: 1,
  dimensions: [{ ID: 1, code: 'default', name: '默认分类' }] as any[],
})
...
function changeClickHandler(value: any) {
...
async function saveClickHandler() {
  ...
  try {
    const res = await axios.put(`/ui/catalogpriv/id/${props.id}/type/${props.type}/contentprivs`, {
...
async function applyToChildCatalog() {
  ...
  const parentPrivMap: Record<string, any> = {}
  const updatedPermissions: Record<string, any> = {}
  ...
  if (props.id) {
    ...
    if (currentSiteID.value && siteListTreeData.value && siteListTreeData.value.length > 0) {
      const datas = await Promise.all([
        axios.get(`/ui/catalogpriv/id/all/type/${props.type}/contentcolumns`),
        axios.get(`/ui/catalogpriv/id/${props.id}/type/${props.type}/contentprivs`, {
          params: {
      ...
      // 定义各列的宽度，使得文字显示比较完整
      // const columnsWidth = [150, 80, 80, 80, 80, 80, 80, 80, 80, 80, 120, 120, 120, 120, 120, 80]
...
<template>
  <div>
    <div class="card flex-align-center">
      ...
      维度：<ElSelect v-model="dimensionID" placeholder="维度视图" style="width:155px;margin-right: 20px;" @change="changeClickHandler">
